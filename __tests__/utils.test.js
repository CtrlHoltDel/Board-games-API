...
describe('Validation', () => {
  ...
  describe('All reviews', () => {
    ...
    it('When passed a query with an invalid key name returns a rejected Promise', () => {
      expect(validate.allReviews({ invalid_key_name: 'cars' })).rejects.toEqual(
        rejectedPromise
      );
    });
  ...
  describe('Add comment', () => {
    it('When passed a parameter with an invalid format, returns a rejected promise', () => {
      ...
      expect(validate.addComment(undefined, 'not undefined')).rejects.toEqual(
        expectedError
      );
      expect(validate.addComment('legitimate name', undefined)).rejects.toEqual(
        expectedError
      );
    });
